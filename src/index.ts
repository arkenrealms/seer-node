...
function updateGames() {
  for (const server of evolutionServers) {
    for (const key in Object.keys(server)) {
      if (!games.evolution.realms[server.key]) games.evolution.realms[server.key] = {}
    ...
    if (!games.evolution.realms[server.key].connection || !games.evolution.realms[server.key].connection.connected) {
      ...
      socket.on('OnBanPlayer', function (msg) {
        try {
          log('Ban', {
          ...
          if (await verifySignature({ value: req.data.address, hash: req.data.signature }, req.data.address) && db.modList.includes(req.data.address)) {
      ...
      socket.on('OnReportPlayer', function (msg) {
        console.log(msg)
      ...
      games.evolution.realms[server.key].connection = socket
    }
  }
}
