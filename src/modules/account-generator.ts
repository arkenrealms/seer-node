...
export async function generateAccounts(app) {
  try {
    ...
    for (let i = app.accountGenerator.startIndex; i < app.accountGenerator.endIndex; i++) {
      ...
      app.accountGenerator.accounts[address] = {
        evolution: sign('evolution').signature,
      }
    }

    jetpack.writeAsync(path.resolve(`./db/claimableAccounts.json`), app.accountGenerator.accounts, {
      atomic: true,
      jsonIndent: 0,
    })
  } catch (e) {
