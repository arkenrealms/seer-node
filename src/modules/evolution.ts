        ...
        if (winners.find(winner => winner.address === player.address)) {
          ...
          if (user.username) { // Make sure cant earn without a character
            ...
            if (req.data.round.winners[0].address === player.address) {
              ...
              await app.live.emitAll('PlayerAction', { address: req.data.round.winners[0].address, username: user.username, message: `${user.username} won ${rewardWinnerMap[0]} ZOD in Evolution!` })
